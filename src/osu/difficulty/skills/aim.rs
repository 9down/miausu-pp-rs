...
impl AimEvaluator {
    const WIDE_ANGLE_MULTIPLIER: f64 = 1.7;
    const ACUTE_ANGLE_MULTIPLIER: f64 = 2.0;
    const SLIDER_MULTIPLIER: f64 = 1.45;
    const VELOCITY_CHANGE_MULTIPLIER: f64 = 1.15;
    ...
    fn evaluate_diff_of<'a>(
            ...
            < 1.25 * osu_curr_obj.strain_time.min(osu_last_obj.strain_time)
        {
                ...
                .zip(osu_last_last_obj.angle)
            {
                ...
                if osu_curr_obj.strain_time > 125.0 {
                    acute_angle_bonus = 0.0;
                } else {
                    let base1 =
                        (FRAC_PI_2 * ((125.0 - osu_curr_obj.strain_time) / 21.0).min(1.0)).sin();

                    let base2 = (FRAC_PI_2
                        * ((osu_curr_obj.lazy_jump_dist).clamp(75.0, 130.0) - 42.0)
                        / 48.0)
                        .sin();
                    ...
                    // * The maximum velocity we buff is equal to 125 / strainTime
                        * angle_bonus.min(150.0 / osu_curr_obj.strain_time)
        ...
        if prev_vel.max(curr_vel).not_eq(0.0) {
            ...
            let dist_ratio = dist_ratio_base.powf(2.2);

            // * Reward for % distance up to 125 / strainTime for overlaps where velocity is still changing.
            let overlap_vel_buff = (150.0 / osu_curr_obj.strain_time.min(osu_last_obj.strain_time))
    ...
    fn calc_wide_angle_bonus(angle: f64) -> f64 {
        (3.5 / 3.5 * ((5.5 / 5.7 * PI).min(angle.max(PI / 5.7)) - PI / 5.7))
            .sin()
            .powf(2.3)
    }

    fn calc_acute_angle_bonus(angle: f64) -> f64 {
        1.5 - Self::calc_wide_angle_bonus(angle)
    }
}
